...
### dashboard/
**Purpose:** The static single-page dashboard and its pre-aggregated JSON. **Not published on the live site since 2026-09-12** — its pairwise chart includes Save-On-Foods, whose prices come from a store in Kamloops, BC (W6). Kept as the record of what Phase 3 published. No backend, no build step — open `dashboard/index.html` over HTTP.
...
**Purpose:** The Netlify build: refuse non-production contexts, run the extract and render gates, then assemble `_site` from `tool/`. **The Phase 3 dashboard is deliberately not published** since 2026-09-12: its pairwise chart compares Save-On-Foods, priced in Kamloops, BC, with Toronto chains (withdrawn as W6).
