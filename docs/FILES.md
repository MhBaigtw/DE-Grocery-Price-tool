...
### docs/CLAIMS.md
**Purpose:** The claims manifest. Every figure in `docs/writeup.md`, `README.md` and `docs/method-note.md`, each with an anchor phrase from its line and the committed query or script that regenerates it — or `derived:` with the arithmetic, `provenance-lost`, or `not-a-figure` with a reason.
**Breaks if removed:** `scripts/check_claims.py` fails, and honesty rule 4 goes back to being enforced by whoever happens to read the draft — which is how 0.196 and the Save-On-Foods 97% were published with no committed source.
**Depends on:** The three published documents; the queries under `analysis/` and the scripts it cites.
**Notes:** Built 2026-09-12 by a sweep of all three documents. The Note column says whether a figure was **confirmed** against its source's saved output or only **traced** through the findings document that quotes it; those are different levels of assurance and the file does not blur them. Anchors pin an entry to a position, so the same digits meaning different things ("8" chains, "8" as a rank) need separate entries, and a reworded sentence fails the check instead of silently keeping a stale source.
...
**Notes:** `Q2e_flag_rank_correlation.sql` was added on 2026-09-12 because the writeup published a rank correlation (0.196) that no committed query produced — a breach of honesty rule 4 found in review. It reproduces 0.196 and shows why it was wrong: Metro and Galleria cannot be measured by promotional text at all, and 0.196 ranked their 0.0 as a measurement. It reports the correlation over the chains measurable both ways alongside it. `Q1`/`Q1b` are the exclusion ledger and bias audit, and they run **before** any finding on purpose — Phase 0's F1 asked the bias question on the wrong axis and got a categorically wrong answer, which was cheap only because nothing had been published yet. `Q1b` exists as a separate file because its whole job is the *control* for `Q1`'s headline: the pooled orphan sale-enrichment of 1.70× is largely a Metro composition effect, and only a within-vendor comparison separates the two.
...
**Notes:** Written after Phase 2 §1.3 recorded the near-miss — paired runs that differed only because one had hit an out-of-memory error under contention. That case was caught because the runs differed; the symmetric case, where both crash identically, was invisible. Order matters: byte-identity is checked **last**, because a crashed pair is a more serious finding than a divergent one and should be reported as such rather than as "not identical". `--expect-statements` pins the result-set count so a query that quietly loses a statement fails rather than being certified reproducible. `--cwd` (2026-09-12) runs the query from another directory, so E1's relative `COPY ... TO 'tool/data/...'` writes a parity extract without touching the committed one.
...
### dashboard/
**Purpose:** The static single-page dashboard and its pre-aggregated JSON. **Retired 2026-09-12, permanently** — kept as a record, never published again (`dashboard/RETIRED.md`): the analysis has one surviving pairwise comparison, the writeup carries it, and every chart is a surface where a withdrawn claim can resurface. The Netlify build refuses to publish it. No backend, no build step — open `dashboard/index.html` over HTTP.
...
### analysis/phase3/
**Purpose:** The query that produced the retired dashboard's data (see `dashboard/RETIRED.md`). Same rule as the earlier phases: no figure exists without a committed query that regenerates it.
...
### analysis/phase4/
**Purpose:** The five queries behind the refresh-cadence measurement, plus `E1_tool_extract.sql`, which builds the tool's static data extract — day-of-week change distribution, price-hold spells, per-chain coverage and absence, staleness by data age and by refresh day, and the same staleness recomputed on the exact basket the tool ships. `R6_save_on_foods_store_ids.sql` reads the Save-On-Foods store id out of each product URL: it is the committed source for the scope correction's store figures (97% of products at store 2210, Kamloops), whose original queries had been deleted.
...
**Notes:** Includes two cases that must **not** fail: the real extract, and a real brand that merely contains "save" (LIFESAVERS). A check too blunt to pass real data gets disabled, so false positives are tested as seriously as misses. Since 2026-09-12 it also proves `--vintage warn` reports a stale extract without refusing it, and relaxes nothing else — a pooled staleness figure is still refused in warn mode.

### scripts/refresh.py
**Purpose:** The **full** refresh, manual since 2026-09-12: fetch both archives, gate on schema, rebuild the full database, run every standing check including the dbt suite and `verify_reproducible`, rebuild the extract twice, gate it, then prove the light path with `check_light_parity.py` and record provenance. It never deploys.
**Breaks if removed:** Archives stop being retained (CLAUDE.md locked decision 2, as amended, puts the mirroring commitment here), the analysis database has no rebuild path, and nothing ever checks that the automated light refresh's filter reproduces the full extract.
**Depends on:** every pipeline script it calls; `node` for the render gate; `scripts/refresh_light.py` for the parity build and the provenance record.
**Notes:** Runs **locally or on a self-hosted runner, not on hosted CI**. The working set is about 10 GB against about 14 GB of runner disk. The site is kept current by `refresh_light.py` instead. `--verify-steps` exists because a dry run printed two commands with wrong flags (`check_model_parity.py` takes no arguments, `run_dbt_tests.py` takes `--target`) and a dry run cannot catch that; it asks each script's own parser instead.

### scripts/refresh_light.py
**Purpose:** The **automated** refresh of the tool's extract. Probes upstream's last-updated stamp against the newest `data/provenance/` record; on a change, downloads only the SQLite archive, loads **only Metro and Walmart** (their full history), runs every gate that guards the extract, writes `tool/data/` and a provenance record, and deletes the archive.
**Breaks if removed:** The site's data goes stale unless someone runs a 10 GB manual rebuild, and the page would then honestly report data that is weeks old.
**Depends on:** `fetch_snapshot.py`, `load_snapshot.py --vendors`, `check_schema.py`, `build_models.py`, the layering/determinism/parity checks, `verify_twice.py --cwd`, `E1_tool_extract.sql`, `check_extract.py`, `test_tool_render.js`, `check_manifest.py`; `psutil` only for `--measure`.
**Notes:** **Cheaper must not mean less guarded:** it runs the schema contract, the model row-count contracts, E1 twice with its in-SQL reliable-tier assertion, the extract gate and the render gate. It does not run the dbt suite or `verify_reproducible`; those stay with full rebuilds, together with the parity proof. Before loading anything it **asserts its vendor filter equals every vendor list in E1 and the extract gate's allowed chains**, so a chain added to one and not the other refuses rather than shipping a silently smaller extract. One half of E1's reliable-tier assertion ("no offers from other chains") is true by construction on a light load; the other half (both chains still reliable tier) still bites. Uses the SQLite archive, not the half-size CSV, because a CSV has no column types and the announced `product_id` type change would pass the schema contract unseen. **Since 2026-09-13 every upstream request is validated and retried at most three times** (0, +15, +45 minutes, capped in code), after the first run was answered with a bot-verification challenge page (findings §11). Measurements and the per-refresh repository growth are in findings §10.

### scripts/test_upstream_validation.py
**Purpose:** Proves the upstream validation and retry. It serves a bot-verification challenge page (an excerpt of the one a GitHub runner received on 2026-09-13) from a local HTTP server, alongside genuine answers, and asserts all of the following. The probe refuses anything that is not a date stamp. The fetch refuses anything not served as a zip with a zip header, and anything truncated, leaving no file. A refusal is attempted exactly three times, then stops. A longer schedule is refused. A challenge that clears on the third attempt succeeds. And end to end, a challenged probe decides and records nothing, and a challenged refresh writes no archive and no manifest.
**Breaks if removed:** The defects from the first run — a challenge page read as a new publication, and saved under an archive's name — could return unnoticed.
...
**Depends on:** `scripts/refresh_light.py`, `git`, `gh` (optional, for the in-progress check and the alert), Python with duckdb, Node; the task registered by `register_fallback_task.ps1`.
**Notes:** **Runs only while that PC is on and its user is logged on.** Before running it refuses unless it is on `main` with a clean tree and no unpushed commits, holds if a workflow refresh is queued or running, and pulls first so the trigger sees the newest provenance record. After building, it discards its result if `origin/main` already carries a record of the same upstream publication. Failures open or comment on the same issue as the workflow. Logs to `logs/refresh_fallback-*.log`.

### scripts/register_fallback_task.ps1
**Purpose:** Registers (or with `-Unregister`, removes) the Windows Task Scheduler task that runs `refresh_fallback.py` daily at 11:00 local time.
**Breaks if removed:** The fallback has to be registered by hand, with settings nobody wrote down.
**Depends on:** Windows Task Scheduler cmdlets; the python.exe path passed as `-Python`.
**Notes:** Interactive logon: no password is stored, and nothing runs at the lock screen or after logout. `StartWhenAvailable` runs a missed start once the PC is next available. 11:00 at UTC+3 is 08:00 UTC, after the workflow's 05:00 run and its retry window.

### docs/diagnostics/
**Purpose:** The probe code behind findings §11: the GitHub workflow that ran on a deleted branch across three runners, and the Netlify Function that ran from AWS on a temporary site.
**Breaks if removed:** The "0 of 80 requests challenged" record loses the code that produced it, and the test cannot be repeated the same way.
**Depends on:** nothing; kept as a record and deliberately outside `.github/workflows/`, so GitHub never runs it.
**Notes:** Both use the project's honest user agent and test archives with 4-byte range requests only. Neither disguises the client or attempts a challenge.

### scripts/check_light_parity.py
**Purpose:** Asserts a full rebuild's extract is **byte-identical** to the light extract for the same snapshot — `products.json` and `history.json` byte for byte, `meta.json` field for field except `built_utc`. Run by `refresh.py` at the end of every full rebuild; a mismatch fails loudly.
**Breaks if removed:** The light path's filter becomes an assumption. A filter that drops rows the extract needs does not crash; it ships a quietly different extract that passes every floor and the render test.
**Depends on:** `scripts/refresh_light.py --snapshot`, `tool/data/`.
**Notes:** Refuses to compare extracts from different snapshots, because a difference between them proves nothing. `--light-dir` compares an already-built extract without rebuilding.

### scripts/test_check_light_parity.py
**Purpose:** Proves `check_light_parity.py` refuses every way two extracts can differ — a dropped product, a single changed byte in history, a changed `meta.json` field, a missing file, different snapshots — and does not refuse `built_utc` alone.
**Breaks if removed:** The only proof the light filter is safe becomes an untested assertion.
**Depends on:** `scripts/check_light_parity.py`, `tool/data/`.
**Notes:** Runs on copies of the committed extract in seconds; the end-to-end parity build is demonstrated in findings §10.

### scripts/measure_repo_growth.py
**Purpose:** Measures what each committed refresh adds to the repository: extracts for successive snapshots are committed in date order into a fresh standalone repository, and it reports the compressed size of each commit's objects and the pack growth after `git gc --aggressive`.
**Breaks if removed:** Committing `tool/data/` on every publication was accepted on the condition that each refresh costs no more than a few hundred KB; without this, that condition is a guess and nobody can re-check it as the extract grows.
**Depends on:** `git`; extract directories built by `refresh_light.py --snapshot`.
**Notes:** Measured 161 KB and 198 KB per refresh on two intervals (findings §10). A fresh repository isolates the data's growth from the rest of this repository's history. It does not measure GitHub's own server-side packing.

### data/provenance/
**Purpose:** One JSON record per extract that shipped: snapshot id, which path built it (`light` or `full`), download timestamps, sha256 and size of each archive, upstream's last-updated stamp, whether the archive was retained, the light load's row counts, and the extract date.
**Breaks if removed:** A light refresh's extract could no longer be tied to the archive it came from — the only provenance it keeps, since it retains no archive (CLAUDE.md locked decision 2, amended 2026-09-12). The automated trigger also loses its baseline and would refresh on every probe.
**Depends on:** written by `scripts/refresh_light.py` (light) and `scripts/refresh.py` (full).
**Notes:** A **directory entry**; files are generated and committed alongside the extract they describe, never hand-edited. The first record, `20260911T200435Z.json`, was written from that full snapshot's manifest so the trigger started from the extract actually live.
...
**Purpose:** The Netlify build: refuse non-production contexts, run the extract and render gates, then assemble `_site` from `tool/`. **The Phase 3 dashboard is retired**, and retirement is enforced: the build fails if `_site/dashboard` exists.
...
**Notes:** Clears the publish directory **before** the gates, so a refusal cannot leave a publishable `_site` behind. Picks a Python interpreter that actually **runs** rather than one merely on `PATH` — Windows ships a `python3` stub that prints an advert and exits 0, which made the gate appear to pass while executing nothing. Both refusal paths are demonstrated: a `deploy-preview` context and a missing `products.json` each exit 1 and leave no publish directory. Since 2026-09-12 the extract's **age limit is enforced only when `tool/data/` changed** since the last build (`netlify_changes.sh data-changed`); every other gate runs on every build.

### scripts/netlify_changes.sh
**Purpose:** Decides two things from Netlify's `CACHED_COMMIT_REF` and `COMMIT_REF`: whether anything that ships changed (`ignore`, for `netlify.toml`), and whether `tool/data/` changed (`data-changed`, for the age limit in `netlify_build.sh`).
**Breaks if removed:** The age limit goes back to blocking every push once the data ages — documentation commits and interface fixes included — which is a gate on the wrong thing and the kind that gets switched off.
**Depends on:** `git`, Netlify's build environment variables.
**Notes:** **Fails safe in both directions**: no cached ref, the same commit rebuilt, or a commit missing from the clone all mean "build" and "data changed". An unknown never skips a build and never relaxes a gate.

### scripts/test_netlify_changes.py
**Purpose:** Proves `netlify_changes.sh` against real commits in this repository: a documentation-only commit skips, a data commit builds and enforces the age limit, an interface-only commit builds and only reports it, and every "cannot tell" case fails safe.
**Breaks if removed:** A change that skipped a real build, or relaxed the age limit on a data deploy, would ship unnoticed.
**Depends on:** `scripts/netlify_changes.sh`, git history, Git's bash on Windows.
**Notes:** Fixtures are found in history by what each commit touched, so the test keeps working as history grows. On Windows it prefers Git's `bash.exe`, because a bare `bash` can resolve to WSL.
...
### .github/workflows/
**Purpose:** `refresh.yml` — the automated refresh, **triggered by upstream publishing, not by a clock**. A cheap daily `probe` job compares upstream's last-updated stamp with the newest `data/provenance/` record and checks the live extract's age; a `refresh` job runs `scripts/refresh_light.py` only when the stamp changed, and commits `tool/data/` and the provenance record to `main`. Netlify builds that push through its own gates.
**Breaks if removed:** The site's prices age until someone runs a manual rebuild, and a stopped pipeline is silent — the failure brief 4.5 names by name.
**Depends on:** `scripts/refresh_light.py`, `scripts/check_extract.py`; a standard hosted runner (14 GB SSD, 16 GB RAM, free for public repositories).
**Notes:** A **directory entry**, holding one workflow. It replaced `probe.yml` on 2026-09-12, which only opened an issue for a weekly manual refresh; that probe ran once, on schedule, successfully. `deploy.yml` was removed earlier when the deploy target moved to Netlify, and its failed run (Pages was never enabled) is still in the public Actions history. Daily pulling of the archive regardless of publication was rejected: about one percentage point of staleness for seven times the maintainer's bandwidth. Failures and an over-age live extract **open or update an issue** rather than going quietly red. The refresh job needs `contents: write`; that this is honoured under the repository's workflow-permission setting is **unverified until its first run**. Its first run (2026-09-13) was refused by a bot-verification challenge from the upstream host; the job timeouts now cover the retry window, a refused probe opens the same issue, and a failed refresh logs its egress address.
...
**Notes:** Pulls the page's script **out of `index.html` itself** rather than keeping a copy, so the test cannot drift from what ships. It has already caught three real defects: a landing state that explained the store-brand limit without using the words brief §3.3 requires, a comparison sentence printing the raw key `SaveOnFoods`, and three chains joined as "A and B and C". **The price-basis rule is conditional since 2026-09-12:** while every offer is `each` the page omits the word, and the test fails if any other basis appears without its label — and, once one exists, if any price at all is unlabelled. Demonstrated to fail with a per-weight offer on a page that never labels. **Since 2026-09-12 it renders the page as if viewed on later days** (0, 1, 3, 10 and 20 days after the update): every price past the measured horizon *today* must carry a warning stating that age, no comparison may stand unless both prices are within the window *today*, the landing state must state the data's age and warn past the horizon, and the page may promise no cadence. Demonstrated to fail on the previous page, which judged ages against the build date and said "updated weekly". Takes an optional tool directory argument for that demonstration.
...
**Notes:** A **directory entry** — one hand-written HTML file and three generated JSON files. The JSON is never hand-edited, so a figure cannot drift from the analysis. The interface is built **unavailable-first**: the "nothing to compare" and "only one chain" states were written before the comparison state, because a large share of products land in them (1,527 of 3,465 in the two-chain build), and the one-chain state carries copy whose only job is to stop a lone price reading as a verdict. `staleness_exceeds_measured` renders as a **bordered warning naming the age**, never a dash. **Ages are judged against the visitor's today** since 2026-09-12: the extract's `days_behind` is an age at the last update, so the page adds the extract's own age before warning, withholds (never adds) a comparison whose prices have aged past 7 days, states "as of <date> — N days ago", warns once the data passes the 14-day measured horizon, and no longer says "updated weekly". Since 2026-09-12 it compares **Metro and Walmart only** — Save-On-Foods is priced in Kamloops, BC — and is simplified mobile-first to search, answer, then caveats in smaller type; every render-test rule still passes, and the disclosures kept at a wording cost are listed in findings §9. The extract is where the tool's honesty is enforced rather than the interface: there is **no pooled staleness figure anywhere in it** (pooling understates Save-On-Foods by over 9 pp), every product carries a `comparison` object naming exactly which chains were compared and which were not with dates and reasons, and per-chain observed date and staleness are stored fields rather than something the page derives. Staleness beyond 14 days is **null with an explicit flag**, not the 14-day figure stretched to fit — 1,137 of 6,093 offers are in that state. 254.7 KB gzipped in total, so history did not have to be shortened.
...
**Validation (2026-09-13):** `fetch_stamp` refuses any answer that is not a date stamp, and `fetch_archive` refuses any answer not served as a zip with a zip header — checked before a file exists — and deletes a truncated body. Both raise `UpstreamRefused` and name what came back (for example, "a bot-verification challenge page"). Added after a GitHub runner saved exactly such a page under the archive's name (findings §11).
...
**Purpose:** Extracts every figure from the three published documents and asserts each is covered by a `docs/CLAIMS.md` entry whose source is a git-tracked file. Also fails on stale entries, anchors that do not contain their figure, and provenance-lost or not-a-figure entries with no note. `--inventory` lists every figure occurrence.
**Breaks if removed:** A published figure with no committed source goes unnoticed until a person catches it. One already did: the 0.196 rank correlation was found by review, not by any check.
**Depends on:** `git` on PATH, `docs/CLAIMS.md`. Standard library only.
**Notes:** A **manifest with anchors rather than inline tags**, so the published documents stay clean and a figure inside a table can be covered without breaking the table. Strips things that are not measurements before extracting: dates, years, snapshot ids, inline code, section and item identifiers, list numbering. **What it does not do:** it checks that a source exists and is committed, not that it produces the number; it sees digits only; and it does not cover the findings documents or `docs/upstream-feedback.md`. Reconfigures stdout to UTF-8 because the documents contain minus signs and × that crashed the first run on a Windows console.

### scripts/test_check_claims.py
**Purpose:** Proves `check_claims.py` fails when it should. Six failure cases, each run against a scratch copy of the documents, manifest and sources — an unsourced figure added to the writeup, a figure edited without its entry, a source path that does not exist, a source file deleted, a provenance-lost entry with no note, an anchor without its figure — plus the real repository, which must pass.
**Breaks if removed:** The claims check becomes an untested assertion.
**Depends on:** `scripts/check_claims.py`, `docs/CLAIMS.md`, `analysis/`, `scripts/`.
**Notes:** The case worth reading is the first: a new sentence with a new percentage and no manifest entry, which is exactly how the 97% was published.
