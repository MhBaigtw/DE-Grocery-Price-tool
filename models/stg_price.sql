...
SELECT
    ...
    p_unit_price(r.current_price)             AS unit_price,
    ...
    p_normalization(r.current_price)          AS normalization,
    p_confidence(r.current_price)             AS parse_confidence,

    -- old (struck-out) price, parsed by the same rules
    r.old_price                               AS old_price_raw,
    p_offer_type(r.old_price)                 AS old_offer_type,
    p_unit_price(r.old_price)                 AS old_unit_price,
    p_price_basis(r.old_price)                AS old_price_basis,
    p_basis_stated(r.old_price)               AS old_price_basis_stated,
    p_normalization(r.old_price)              AS old_normalization,
    p_confidence(r.old_price)                 AS old_parse_confidence,
